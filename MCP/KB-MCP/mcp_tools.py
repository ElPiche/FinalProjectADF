...
# Compatibility wrapper: many existing calls in this module use the ordering
# log_message(message, level, component, method, **kwargs)
# while utils.log_message expects (level, component, method, message, **kwargs).
# Rebind log_message here to preserve the existing call sites without changing them.
def log_message(message: str, level: str = "info", component: str = "mcp_tools", method: str = "entry", **kwargs):
    """
    Adapter that forwards to utils.log_message with the expected parameter order.
    """
...
# Debug helper: ping Elasticsearch using several candidate hosts and log results.
# This is added as a top-level helper and invoked on module import for quick visibility
# in container logs. It does not change the existing elasticsearch_sql implementation.
def ping_elasticsearch_debug():
    """Try multiple ES host variants, log to structured logger and console."""
    request_id = str(uuid.uuid4())[:8]
    try:
        import db
        hosts_to_try = []
        # Primary configured host (may be a single host or comma-separated)
        try:
            primary = db.es_host
            if primary:
                # split if multiple comma-separated hosts are provided
        ...
        # Fallback logging if db import or other unexpected error occurs
        try:
...
# Run the ping helper on import so Docker container logs show immediate status.
try:
    ping_elasticsearch_debug()
except Exception:
    # Avoid import-time failures
    pass

# Expose ping helper as an MCP tool for on-demand debugging
@mcp.tool()
def ping_elasticsearch() -> str:
    """MCP tool wrapper for ping_elasticsearch_debug - returns JSON status."""
    request_id = str(uuid.uuid4())[:8]
    start = time.time()
    try:
