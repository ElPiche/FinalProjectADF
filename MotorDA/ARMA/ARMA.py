...
# print(f"this is df after filling missing timestamps with zeroes: {df}")
...
# print(f"this is df after filling NaN rows with zeroes: {df}")
...
# print(f"this is df after converting to integer with zeroes: {df}")

# Create a complete hour range
full_range = pd.date_range(start=df["es_timestamp"].min(), end=df["es_timestamp"].max(), freq="60min")

print(f"this is the length of DF: {len(df)}")
print(f"this is the length of full_range: {len(full_range)}")

# print(f"this is full range: {full_range}")


# for instance in df:
#     if (instance["es_timestamp"].time)
#         print(x)


# Ensure consistent timestamp format
df["es_timestamp"] = [pd.Timestamp(timestamp) for timestamp in df["es_timestamp"]]

# Find missing timestamps and fill with zeros
missing_range = [timestamp for timestamp in full_range if timestamp not in df["es_timestamp"].values]
...
df['status_code_200_counter'] = pd.to_numeric(df['status_code_200_counter'], errors='coerce').fillna(0)
df['status_code_5xx_counter'] = pd.to_numeric(df['status_code_5xx_counter'], errors='coerce').fillna(0)
...
def fit_arma_model(data, column_name):
    ...
    if adf_result[1] < 0.05:
        ...
        print(f"Series is not stationary, using differenced data")
        series_to_use = series_diff
        #print(f"this is series to use: {series_to_use}")
        #print(series_to_use["status_code_200_counter"])
        plt.subplot(2, 2, 1)
        plt.plot(series_to_use.index, series_to_use, color='magenta', marker='o', mfc='pink', markersize=3)
    ...
    try:
        ...
        for p in range(0, 4):  # Reduced from 6 to 4
            for q in range(0, 4):  # Reduced from 6 to 4
                try:
                    ...
                    if p + q <= 2:
                        fitted_model = model.fit(method='lbfgs', cov_type='robust')
                    else:
                        fitted_model = model.fit(method='mle', cov_type='robust')
                    ...
                    if "singular matrix" not in error_msg and "convergence" not in error_msg:
                        print(f"Warning: ARMA({p},{q}) failed: {str(e)[:50]}...")
                    continue
        ...
        if best_model:
            ...
            if lb_pvalue < 0.05:
                print("⚠️  WARNING: Residuals show autocorrelation - model may be inadequate")
                print("   Consider: Higher order ARMA, seasonal terms, or different model")
                ...
                try:
                    ...
                    arima_fitted = arima_model.fit(method='lbfgs', cov_type='robust')
                    
                    arima_residuals = arima_fitted.resid
                    arima_ljung = acorr_ljungbox(arima_residuals, lags=min(10, len(arima_residuals)//5), return_df=True)
                    arima_lb_pvalue = arima_ljung['lb_pvalue'].iloc[-1]
                    
                    if arima_lb_pvalue > lb_pvalue:
                        print(f"   ARIMA({p_arima},1,{q_arima}) model performs better: p-value = {arima_lb_pvalue:.4f}")
...
plt.subplot(2, 2, 1)
plt.plot(df.index, df["status_code_200_counter"], color='magenta', marker='o', mfc='pink', markersize=3)
...
plt.subplot(2, 2, 2)
plt.plot(df.index, df["status_code_5xx_counter"], color='red', marker='o', mfc='orange', markersize=3)
...
def correlation_analysis(df):
    ...
    correlation = df[['status_code_200_counter', 'status_code_5xx_counter']].corr()
    print("\nCorrelation Matrix:")
    print(correlation)
    
    # Plot correlation
    plt.figure(figsize=(8, 6))
    plt.scatter(df['status_code_200_counter'], df['status_code_5xx_counter'], alpha=0.6)
...
def validate_model(data, column, test_size=0.2):
    ...
    try:
        model = ARIMA(train_data, order=(2, 0, 2))  # Simple ARMA(1,1)
